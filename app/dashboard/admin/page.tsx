...
export default async function AdminDashboardPage() {
  const [clients, tasks, salesmen] = await Promise.all([
    prisma.client.findMany({ select: { status: true } }),
    prisma.task.count(),
    prisma.user.findMany({
      where: { role_id: 3 },
      include: { assignedClients: { select: { status: true } } },
    }),
  ]);
